  ...
  $.fn.faba = function(options){
	  ...
	  function lastPage(id){
	    ...
	    function loadAlbumsSort(result, paginate, num, paged, dir, last, cached){
			...
			if( albumsScroll !== false ) $('#fb-breadcrumbs').velocity('scroll', {duration: albumsScroll});
	    ...
	    function afterFirstLoadInit(id, photos, images, last){	
			...
			$('#phnx, #phpr').off().on('click', {i: id}, photosNavigation );
			...
			if( images.paging && images.paging.next || images.next ){
				if( zbir[id] > photosPerPage ) {
					$('#phnx').velocity('fadeIn', {duration: 150});
	    ...
	    function afterLoadPhotos(id, container, cached) {
	    	...
	    	container.find('.imgdiv').waitForImages(true).done(function(){ 
				...
				var num = parseInt( paged.text() );
		    	container.find('a').velocity(anm['photos']['in'],{stagger: 26, 
		    			begin: function(){ 
		    				$(this).find('.hidden').removeClass('hidden');
		    				container.find('.fbp-overlay').remove();
		    				if( photosSroll !== false ) $('#fb-breadcrumbs').velocity('scroll', {duration: photosSroll});
		...
		function firstLoadPhotos(e){ 
			...
			var load = [
				{ elements: $('#faba-inner'), properties: anm['albumsLoad']['out'], options: {duration: anm['albumsLoad']['duration'] } },
				{ elements: container, properties: { width: '20%' }, options: {sequenceQueue: false, duration: lineSequence} },
			    { elements: container, properties: { width: '30%' }, options: {duration: lineSequence} },
			    { elements: container, properties: { width: '50%' }, options: {duration: lineSequence} },
			    { elements: container, properties: { width: '100%' }, options: {duration: lineSequence} },
			    { elements: container, properties: { height: '100%', top: 90 },
					options : {
						duration: photosContainerLoad,
						complete: function (){ 
							container.css({position: 'relative', padding: 15, margin: 'auto', left: 0, top: 0});
	    ...
	    function photoMarkup(value, result, t, cached, num) { 
	  		if( value == '' ) value = {images: {0: '', 1: {source: ''}, length: 2},id: '', name: ''}
			...
			if( !t ) { 
				...
				if( result.data && t === result.data.length || cached ){
					...
					container.find('.imgdiv').waitForImages(true).done(function(){ 
						...
						container.find('a').velocity('stop').velocity(anm['photos']['in'], {stagger: 32,
							complete: function(){
								var isCached = cached ? true : false;
	  ...
	  function loadPhotos(url, paged, num, dir, last, h, cached, isAuto) { 
			...
			var ldh = [
				{elements: overlay, properties: 'transition.fadeIn', options: {duration: 360} },
				{elements: $('.fbp-container').find('a'), properties: anm['photos']['out'], 
				  options: {
					sequenceQueue: false,
					stagger: 5,
					complete: function(){
	  ...
	  var hoverHandle = {
	  	 photo: {
	  	 	over: function(el){
	  	 		var $this = el;
		    	var load = [
					{ elements: $this.find('.imgdiv'), properties: {scaleX: 1.23, scaleY: 1.23, rotateZ: 6 }, options: {duration: anm['photoHover']['image']['duration']} },
					{ elements: $this.find('.ovlpic'), properties: anm['photoHover']['overlay']['in'], options: {sequenceQueue: false, duration: anm['photoHover']['overlay']['duration']} },
					{ elements: $this.find('.lupa'), properties: anm['photoHover']['button']['in'], options: {sequenceQueue: false, duration: anm['photoHover']['button']['duration']} }
				];
	  	 	...
	  	 	out: function(el){
		    	...
		    	$this.find('.imgdiv').velocity('stop');
		  		$this.find('.imgdiv').velocity('reverse', {duration: 400});
		  		$this.find('.ovlpic').velocity('stop');
		  		$this.find('.ovlpic').velocity('reverse', {duration: anm['photoHover']['overlay']['duration']});
		  		$this.find('.lupa').velocity('reverse', {duration: anm['photoHover']['button']['duration']});
	  	 	}
	  	 },
	  	 album: {
	  	 	over: function(el){
	  	 		var $el = el;
	  	 		var hvr = [
					{ elements: $el.find('.imgdiv'), properties: anm['albumsHover']['image']['out'], options: {duration: anm['albumsHover']['image']['duration']} },
					{ elements: $el.find('.ovlpic'), properties: anm['albumsHover']['overlay']['in'], options: {sequenceQueue: false, duration: anm['albumsHover']['overlay']['duration']} },
					{ elements: $el.find('a'), properties: anm['albumsHover']['button']['in'], options: {sequenceQueue: false, duration: anm['albumsHover']['button']['duration']} },
					{ elements: $el.find('.badge'), properties: anm['albumsHover']['badge']['in'], options: {sequenceQueue: false, duration: anm['albumsHover']['badge']['duration']} }
				];
	  	 	...
	  	 	out: function(el){
		  		...
		  		$el.find('.ovlpic').velocity('reverse', {duration: anm['albumsHover']['overlay']['duration']});
		  		$el.find('.badge').velocity('stop');
		  		$el.find('.badge').velocity('reverse', {duration: anm['albumsHover']['badge']['duration']});
		...
		var photos = $('#fb-photos');
	  	if( container.find('a').length > 0 && !singleAlbum ) {
			...
			paginate.find('li').remove();
		  	...
		  	container.css({padding: 0, width: '100%', left: 0, position: 'absolute'});
			container.velocity({height: 10}, {duration: photosContainerLoad, sequenceQueue: false, 
				complete: function(){
					var load = [
						{ elements: container, properties: {width: 0}, options: {duration: lineLoad} },
						{ elements: albumsInner, properties: anm['albumsLoad']['in'], 
						    options: {
						    	duration: anm['albumsLoad']['duration'],
						    	complete: function(){
						    		photos.css('display', 'none');
									var paginate = $('#pagination');
									if( albumsScroll !== false ) breadcrumbs.velocity('scroll', {duration: albumsScroll});
									if( albumsPgd > 1 ){
									...
									setTimeout(function(){albWall.refresh();},120);
							    }
							} 
						}
					];
	    	...
	    	albumsInner.find('.imgdiv').waitForImages(true).done(function(){
		        ...
		        if( albums.paging && albums.paging.next || albums.next ){
		        	...
		        	if( albumsPgd > 1 ) {
			        	...
			        	$('#aphnx').velocity('fadeIn', {duration: 600});
						photosNumeration(albumsPgd);
					}
		    	}	
		    	if( cached ) {
